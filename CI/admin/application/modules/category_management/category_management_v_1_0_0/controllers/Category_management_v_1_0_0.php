...
class Category_management_v_1_0_0 extends MY_Controller{
	...
	public function category_management_reg_in(){
		...
		if($result == 0) {
			echo json_encode(array('code' => 0, 'msg' => 'Other categories cannot be registered.'));
			exit;
		} else {
	...
	public function category_management_del(){
		...
		if($result == 0) {
			echo json_encode(array('code' => 0, 'msg' => 'Deletion failed.. Please contact the administrator'));
			exit;
		} else {
	...
	public function category_management_mod_up(){
		...
		$category_name_us = $this->_input_check('category_name_us',array());
		$category_name_kr = $this->_input_check('category_name_kr',array());
		$category_name_bd = $this->_input_check('category_name_bd',array());
		...
		$data["category_name_us"] = $category_name_us;
		$data["category_name_kr"] = $category_name_kr;
		$data["category_name_bd"] = $category_name_bd;
		...
		if($result == 0) {
			echo json_encode(array('code' => 0, 'msg' => 'Edit failed. Please contact the administrator'));
			exit;
		} else {
			echo json_encode(array('code' => 1));
			exit;
	...
	public function category_state_up(){
		...
		if($state =="1"){
			...
			if($check>0){
				echo json_encode(array('code' => "-1", 'msg' => 'It is in the product in the category.'));
				exit;
			}
		}
  ...
  //이미지 수정
	...
	public function img_change_mod_up(){
		...
		if($result == '0'){
 			$response->code = "0";
 			$response->code_msg = "Please try again.";
 		}else{
 			$response->code = "1";
 			$response->code_msg = "The image has been saved.";
 		}
