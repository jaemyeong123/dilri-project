...
class Board_v_1_0_0 extends MY_Controller{
	...
	public function board_detail(){
		$board_idx = $this->_input_check("board_idx",array("empty_msg"=>"The announcement key is missing."));//공지사항 키가 누락되었습니다.
		$history_data = $this->_input_check("history_data",array());
	...
	public function best_yn_mod_up(){
		...
		if ($best_yn=='Y') {
			...
			if ($best_yn_cnt>=10) {
				$response->code = '-1';
				$response->code_msg = "You can specify up to 10 BEST community posts. Release and re-designate the specified item.";//BEST 커뮤니티글은 최대 10개까지 지정 가능 합니다. 지정된 항목을 해제 후 다시 지정하여 주세요.
